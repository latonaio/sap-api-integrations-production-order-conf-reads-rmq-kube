...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-production-order-conf-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) ConfByOrderID(orderID string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": confbyOrderIDData, "function": "ProductionOrderConfirmationConfirmation"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": materialMovementsData, "function": "ProductionOrderConfirmationMaterialMovements"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": batchCharacteristicData, "function": "ProductionOrderConfirmationBatchCharacteristic"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) ConfByOrderIDConfGroup(orderID, confirmationGroup string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": confbyOrderIDConfGroupData, "function": "ProductionOrderConfirmationConfirmation"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) ConfByOrderIDSeqOp(orderID, sequence, orderOperation string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": confbyOrderIDSeqOpData, "function": "ProductionOrderConfirmationConfirmation"})
	if err != nil {
		c.log.Error(err)
		return
	}
